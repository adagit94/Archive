...
export type WithRequiredRecursive<T extends object> = {
  [k in keyof T]-?: T[k] extends object ? WithRequiredRecursive<T[k]> : T[k];
};
...
export default class TypesUtils {
  ...
  public static isPrimitive(val: unknown): val is PrimitiveValue {
    return TypesUtils.isString(val) || this.isNumber(val) || TypesUtils.isBoolean(val);
  }

  public static containsOnlyPrimitives(vals: unknown[]): vals is PrimitiveValue[] {
    return vals.every(TypesUtils.isPrimitive);
  }
  ...
  public static hasStringProperties<X, Y extends PropertyKey>(
    obj: X,
    propertyNames: Array<Y>,
  ): obj is X & Record<Y, string> {
    return obj instanceof Object && propertyNames.every((prop) => this.hasStringProperty(obj, prop));
  }
